...
print("-" * 40)
print("видим, что интерфейс у всех одинаковый")
repositories = {
    "JSON файл": json_repo,
    "YAML файл": yaml_repo,
    "База данных": db_repo
...
for name, repo in repositories.items():
    ...
    clients = repo.get_k_n_short_list(2, 1)
    for i, client in enumerate(clients, 1):
        print(f"  {i}. {client.get_info()}")


print("-" * 40)
print("валидация есть")
valid_test = [
    {
        "last_name": "123",
        "first_name": "Тест",
    ...
    {
        "last_name": "Корректный",
        "first_name": "Клиент",
        "phone": "+79161234567",
        "address": "г. Москва"
    }
]
for i, valid_test in enumerate(valid_test, 1):
    print(f"  Данные: {valid_test['last_name']} {valid_test['first_name']}, тел: {valid_test['phone']}")
    result = db_repo.add_client(
        last_name=valid_test['last_name'],
        first_name=valid_test['first_name'],
        phone=valid_test['phone'],
        address=valid_test['address']
    )

print("-" * 40)
print("полиморфизм есть")
def polymorphism(repositories_dict):
    for name, repo in repositories_dict.items():
        print(f"\n{name}:")
        count = repo.get_count()
        print(f"  Всего клиентов: {count}")
        client = repo.get_by_id(1)
        if client:
            print(f"  Клиент №1: {client.last_name} {client.first_name}")
        page = repo.get_k_n_short_list(3, 1)
